...
source "amazon-ebs" "autogenerated_1" {
  ...
  ami_name      = "vector-custom-${local.timestamp}"
...
build {
  ...
  provisioner "shell" {
    inline = [
      "sleep 10", # give time to the AMI to update, makes the build less flaky
      "mkdir -p /tmp/builder/",
    ]
  }
  ...
  provisioner "file" {
    sources = [
      "./terraform/vector/vector.toml",
    ]
    destination = "/tmp/builder/"
  }

  provisioner "shell" {
    script = "./terraform/vector/provision.sh"
  }
}
